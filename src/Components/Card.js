...
export const Card = ({ id, title, tag, name, users }) => {
  ...
  return (
    // Card Component
    <div className="card-flex" style={{ gap: "5px" }}>
      <div className="flex-sb">
        {groupBy === "priority" && <ElementIcon element={ticketStatus} />}
        ...
        {!user && (
          ...
          >
            <img
              ...
              src={`https://ui-avatars.com/api/?name=${initial}&size=100&background=random&color=fff&rounded=true`}
              alt="User"
            />
          </div>
        )}
      ...
      <div className="card-tag">
        ...
        {tag?.map((elem, index) => {
          return (
            <div key={index} className="tags" style={{ color: "grey" }}>
              {" "}
              <span>•</span> {elem}
            </div>
          );
